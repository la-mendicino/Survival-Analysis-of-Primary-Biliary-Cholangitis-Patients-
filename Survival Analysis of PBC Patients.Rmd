...
```
(REWRITE)
The results from LASSO regression give a model with 9 factors: age, ascites, edema, bili, albumin, copper, alk.phos, ast, protime, and stage. LASSO successfully shrunk 8 factors to zero.
...
(REWRITE)
LASSO regression resulted in 8 non-zero factors: age, ascites, edema, lbili, albumin, lcopper, lprotime, and stage.
...
(REWRITE)
The two approaches, using untransformed and log transformed numerical factors, resulted in two different parsimonious sets of variables. The first set included one more variable (ast) and shared the rest of the factors with the second set. 
...
```{r}
s1_model <- data.frame("Standard Model, untransformed", AIC(standard_cox1), 
                       BIC(standard_cox1), 0.849)
names(s1_model) <- c("Model", "AIC", "BIC", "Concordance")

s2_model <- data.frame("Standard Model, transformed", AIC(standard_cox2), 
                       BIC(standard_cox2), 0.859)
names(s2_model) <- c("Model", "AIC", "BIC", "Concordance")

p1_model <- data.frame("Parsimonious Model, untransformed", AIC(parsimonious_cox1), 
                       BIC(parsimonious_cox1), 0.849)
names(p1_model) <- c("Model", "AIC", "BIC", "Concordance")

p2_model <- data.frame("Parsimonious Model, transformed", AIC(parsimonious_cox2), 
                       BIC(parsimonious_cox2), 0.857)
names(p2_model) <- c("Model", "AIC", "BIC", "Concordance")

model_diagnostics <- rbind(s1_model, p1_model, s2_model, p2_model)
model_diagnostics
```

(REWRITE)
In the untransformed approach, AIC, BIC, and concordance all decrease from the standard to parsimonious model. This occurs in the transformed approach too. Between the two approaches, the transformed model exhibits the lowest AIC and BIC, while retaining a concordance greater than the standard untransformed model. 
...
pars1 <- rep("Parsimonious Cox, untransformed", length(parsimonious_cox1_plot$time))
pars_df1 <- data.frame(parsimonious_cox1_plot$time, parsimonious_cox1_plot$surv, pars1)
names(pars_df1) <- c("Time", "Survival", "Model")

plot_combo_df_1 <- rbind(standard_df1, pars_df1)
...
plot_combo_df <- rbind(standard_df1, standard_df2, pars_df1, pars_df2)
